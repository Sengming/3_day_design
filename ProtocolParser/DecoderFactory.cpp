...
BaseDecoderInterface* DecoderFactory::generateDecoder(DeviceType decoderCode) 
{
	...
	switch (decoderCode)
	{
		case CLAMP_MODULE_1:
			...
			//   
			//     with return at end.
			retVal = dynamic_cast<BaseDecoderInterface*>(new Clamp1Decoder());  // BWS You do not need to cast to a base class.
			break;
		case CLAMP_MODULE_2:
			retVal = dynamic_cast<BaseDecoderInterface*>(new Clamp2Decoder());
			break;
		default:
			retVal = NULL;
	}
